...
void ConditionRegeneration::addCondition(Creature*, const Condition* condition)
{
	if (updateCondition(condition)) {
		...
		healthPercentTicks = conditionRegen.healthPercentTicks;
		healthPercentGain = conditionRegen.healthPercentGain;

		manaPercentTicks = conditionRegen.healthPercentTicks;
		manaPercentGain = conditionRegen.healthPercentTicks;
...
bool ConditionRegeneration::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	...
	} else if (attr == CONDITIONATTR_HEALTHPERCENTTICKS) {
		return propStream.read<uint32_t>(healthPercentTicks);
	} else if (attr == CONDITIONATTR_HEALTHPERCENTGAIN) {
		return propStream.read<uint32_t>(healthPercentGain);
	} else if (attr == CONDITIONATTR_MANAPERCENTTICKS) {
		return propStream.read<uint32_t>(manaPercentTicks);
	} else if (attr == CONDITIONATTR_MANAPERCENTGAIN) {
		return propStream.read<uint32_t>(manaPercentGain);
	}
	return Condition::unserializeProp(attr, propStream);
}
...
void ConditionRegeneration::serialize(PropWriteStream& propWriteStream)
{
	...
	propWriteStream.write<uint8_t>(CONDITIONATTR_HEALTHPERCENTTICKS);
	propWriteStream.write<uint32_t>(healthPercentTicks);

	propWriteStream.write<uint8_t>(CONDITIONATTR_HEALTHPERCENTGAIN);
	propWriteStream.write<uint32_t>(healthPercentGain);
	...
	propWriteStream.write<uint8_t>(CONDITIONATTR_MANAPERCENTTICKS);
	propWriteStream.write<uint32_t>(manaPercentTicks);

	propWriteStream.write<uint8_t>(CONDITIONATTR_MANAPERCENTGAIN);
	propWriteStream.write<uint32_t>(manaPercentGain);
}
...
bool ConditionRegeneration::executeCondition(Creature* creature, int32_t interval)
{
	...
	// Mytheos
	internalHealthPercentTicks += interval;
	//std::cout << "Internal Health percent Ticks1: " << internalHealthPercentTicks <<" \n";

	if (creature->getZone() == ZONE_PROTECTION) {
		return ConditionGeneric::executeCondition(creature, interval);
	}

	if (internalHealthTicks >= healthTicks && healthTicks > 0) {
	...
	//std::cout << "Health Percent Ticks: " << healthPercentTicks << " \n";
	// Mytheos
	if (internalHealthPercentTicks >= healthPercentTicks) {
		internalHealthPercentTicks = 0;
		//std::cout << "Internal Health percent Ticks2: " << internalHealthPercentTicks << " \n";

		int32_t realHealthGain = creature->getHealth();
		//std::cout << "Health percent gain: " << healthPercentGain << " \n";
		// float healthPercentGainFloat = static_cast<float>(healthPercentGain);
		float healthPercentGainAux = creature->getMaxHealth() * (healthPercentGain / 100.f);
		creature->changeHealth(healthPercentGainAux);
		//std::cout << healthPercentGainAux << " Regeneracion Porcentual \n";
		realHealthGain = creature->getHealth() - realHealthGain;
	...
	if (internalManaTicks >= manaTicks && manaTicks > 0) {
		internalManaTicks = 0;
	...
	// Mytheos
	if (internalManaPercentTicks >= manaPercentTicks) {
		internalManaPercentTicks = 0;

		if (Player* player = creature->getPlayer()) {
			int32_t realManaGain = player->getMana();

			float manaPercentGainAux = player->getMaxMana() * (manaPercentGain / 100.f);
			player->changeMana(manaPercentGainAux);
			realManaGain = player->getMana() - realManaGain;
...
bool ConditionRegeneration::setParam(ConditionParam_t param, int32_t value)
{
	...
	switch (param) {
		...
		case CONDITION_PARAM_MANAPERCENTGAIN:
			manaPercentGain = value;
			return true;

		case CONDITION_PARAM_MANAPERCENTTICKS:
			manaPercentTicks = value;
			return true;

		case CONDITION_PARAM_HEALTHPERCENTGAIN:
			healthPercentGain = value;
			return true;

		case CONDITION_PARAM_HEALTHPERCENTTICKS:
			healthPercentTicks = value;
			return true;
