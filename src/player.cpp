...
bool Player::setVocation(uint16_t vocId)
{
	...
	if (condition) {
		...
		condition->setParam(CONDITION_PARAM_HEALTHPERCENTGAIN, vocation->getHealthPercentGainAmount());
		condition->setParam(CONDITION_PARAM_HEALTHPERCENTTICKS, vocation->getHealthPercentGainTicks() * 1000);
		condition->setParam(CONDITION_PARAM_MANAGAIN, vocation->getManaGainAmount());
		condition->setParam(CONDITION_PARAM_MANATICKS, vocation->getManaGainTicks() * 1000);
		condition->setParam(CONDITION_PARAM_MANAPERCENTGAIN, vocation->getManaGainAmount());
		condition->setParam(CONDITION_PARAM_MANAPERCENTTICKS, vocation->getManaGainTicks() * 1000);
	}
	return true;
}
...
{
	//Mytheos
	internalManaPercentTicks += interval;
	...
	// Mytheos
	if (internalManaPercentTicks >= getVocation()->getManaPercentTicks() && getVocation()->getId() == 4 && !hasCondition(CONDITION_INCOMBAT)) {
		changeManaByPercent(10);
	}
}
...
void Player::onBlockHit()
{
	...
	// Mytheos
	if (getVocation()->getId() == 4) {
		addInCombatTicks();
		changeManaByPercent(2);
	}
}
...
void Player::addInCombatTicks(bool pzlock /*= false*/)
{
	if (hasFlag(PlayerFlag_NotGainInFight)) {
		return;
	}
	...
	Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_INCOMBAT, g_config.getNumber(ConfigManager::PZ_COMBAT), 0);
	addCondition(condition);
}
	...
	if ((OTSYS_TIME() - lastAttack) >= getAttackSpeed()) {
		...
		if (weapon) {
			if (!weapon->interruptSwing()) {
				...
				if (voc->getId() == 4) {
					changeManaByPercent(voc->getManaPercentPerAttack());
					addInCombatTicks();
				}
...
void Player::addDamagePoints(Creature * attacker, int32_t damagePoints)
{
	Creature::addDamagePoints(attacker, damagePoints);
...
// Mytheos.
void Player::changeManaByPercent(int32_t manaPercentToChange)
{
	if (!hasFlag(PlayerFlag_HasInfiniteMana)) {
		float manaToChangeAux = getMaxMana() * (manaPercentToChange / 100.f);
		if (manaPercentToChange > 0) {
			mana += std::min<int32_t>(manaToChangeAux, getMaxMana() - mana);
		}
		else {
			mana = std::max<int32_t>(0, mana + manaToChangeAux);
		}
	}
