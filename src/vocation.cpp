...
bool Vocations::loadFromXml()
{
	...
	for (auto vocationNode : doc.child("vocations").children()) {
		...
		if ((attr = vocationNode.attribute("gainhppercentticks"))) {
			voc.gainHealthPercentTicks = pugi::cast<uint32_t>(attr.value());
			//std::cout << "se cargo la variable gainhppercentticks " << voc.gainHealthPercentTicks << "\n";
		}

		if ((attr = vocationNode.attribute("gainhppercentamount"))) {
			voc.gainHealthPercentAmount = pugi::cast<uint32_t>(attr.value());
			//std::cout << "se cargo la variable gainhppercentamount \n" << voc.gainHealthPercentAmount << "\n";
		}
		...
		if ((attr = vocationNode.attribute("gainmanapercentticks"))) {
			voc.gainManaPercentTicks = pugi::cast<uint32_t>(attr.value());
		}

		if ((attr = vocationNode.attribute("gainmanapercentamount"))) {
			voc.gainManaPercentAmount = pugi::cast<uint32_t>(attr.value());
		}

		if ((attr = vocationNode.attribute("manapercentperattack"))) {
			voc.manaPercentPerAttack = pugi::cast<uint32_t>(attr.value());
		}

		if ((attr = vocationNode.attribute("manapercentticks"))) {
			voc.manaPercentTicks = pugi::cast<uint32_t>(attr.value());
		}
