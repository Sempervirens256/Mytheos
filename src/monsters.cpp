...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, MonsterType* mType2, const std::string& description)
{
	std::string name;
	...
	if ((attr = node.attribute("min"))) {
		if (mType2->info.attackStat > 0 && false) {
			...
			sb.minCombatValue = static_cast<int32_t>(minDmgAux);
			std::cout << "1 sb.minCombatValue = " << sb.minCombatValue << "\n";
		}
		else {
			sb.minCombatValue = pugi::cast<int32_t>(attr.value());
			std::cout << "2 sb.minCombatValue = " << sb.minCombatValue << "\n";
		}
	}
	...
	if ((attr = node.attribute("max"))) {
		...
		if (mType2->info.attackStat > 0 && false) {
			...
			std::cout << "2 sb.maxCombatValue = " << sb.maxCombatValue << "\n";
		}
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if ((attr = monsterNode.attribute("attackspeed"))) {
		mType->info.attackSpeed = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("casttime"))) {
		mType->info.castTime  = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("magicattack"))) {
		mType->info.magicAttack = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("attackstat"))) {
		mType->info.attackStat = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("dodge"))) {
		mType->info.dodge = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("criticalchance"))) {
		mType->info.criticalChance = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("criticaldamage"))) {
		mType->info.criticalDamage = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("defencestat"))) {
		mType->info.defenceStat = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("magicdefence"))) {
		mType->info.magicDefence = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = monsterNode.attribute("hitstat"))) {
		mType->info.hitStat = pugi::cast<uint64_t>(attr.value());
	}
	...
	if ((node = monsterNode.child("attacks"))) {
		for (auto attackNode : node.children()) {
			spellBlock_t sb;
			if (deserializeSpell(attackNode, sb, mType, monsterName)) {
	...
	if ((node = monsterNode.child("defenses"))) {
		...
		for (auto defenseNode : node.children()) {
			spellBlock_t sb;
			if (deserializeSpell(defenseNode, sb, mType, monsterName)) {
