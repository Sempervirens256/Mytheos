...
CombatDamage Combat::getCombatDamage(Creature* creature, Creature* target) const
{
	...
	if (formulaType == COMBAT_FORMULA_DAMAGE) {
		...
		if (creature->getCombatValues(min, max)) {
			/* Mytheos 
			if (creature->getAttackStat() > 0) {
				float minDmgAux = (creature->getAttackStat() / 100.f) * min;
				float maxDmgAux = (creature->getAttackStat() / 100.f) * max;

				damage.primary.value = normal_random(min + minDmgAux, max + maxDmgAux);
			} else {
			*/
			// Esta pendejada tampoco cambia el daño, puta perdida de tiempo...
			...
			if (params.valueCallback) {
				...
				damage.primary.value = normal_random(
					static_cast<int32_t>((levelFormula * mina + minb) * player->getVocation()->magicDamageMultiplier),
					static_cast<int32_t>((levelFormula * maxa + maxb) * player->getVocation()->magicDamageMultiplier)
				);
