...
enum ConditionAttr_t {
	...
	CONDITIONATTR_SUBID,
	CONDITIONATTR_HEALTHPERCENTTICKS,
	CONDITIONATTR_HEALTHPERCENTGAIN,
	CONDITIONATTR_MANAPERCENTTICKS,
	CONDITIONATTR_MANAPERCENTGAIN,
...
class ConditionRegeneration final : public ConditionGeneric
{
	...
	private:
		...
		//Mytheos mod.
		uint32_t internalHealthPercentTicks = 0;
		uint32_t internalManaPercentTicks = 0;

		uint32_t healthPercentTicks = 1000;
		uint32_t healthPercentGain = 5;

		uint32_t manaPercentTicks = 1000;
		uint32_t manaPercentGain = 0;
